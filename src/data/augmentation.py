...
from .transforms import ComposeWithUsingBoundingBoxes
...
class ComposeWithUsingBoundingBoxesAndImgaugSupport(ComposeWithUsingBoundingBoxes):
    def _single_transform_call_impl(self, transform, img, boxes):
        ...
        image, bounding_boxes = transform(
            image=image, bounding_boxes=bounding_boxes)
        try:
            img = torch.from_numpy(image).permute(2, 0, 1)
        except ValueError as exc:
            # print(f"Warning ! {exc}")
            # workaround in case of negative strides in ndarray
            if "array.copy()" in exc.args[0]:
                img = torch.from_numpy(image.copy()).permute(2, 0, 1)
            else:
                raise
        boxes = torch.vstack([torch.from_numpy(bb.coords.flatten())
                             for bb in bounding_boxes])
        return img, boxes


# All the code below is adopted from here: https://imgaug.readthedocs.io/en/latest/source/examples_basics.html
